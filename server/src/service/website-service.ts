...
export class WebsiteService {
    ...
    static async check(url: string): Promise<WebsiteDetail> {
        ...
        try {
            ...
            if (axiosErr.response) {
                // Server merespon dengan status error (misal 500, 404)
    ...
    static async updateDetailAll(): Promise<WebsiteResponse[]> {
        const websites = await prismaClient.website.findMany();

        const result = await Promise.all(websites.map(async (website) => {
            const start = Date.now();
